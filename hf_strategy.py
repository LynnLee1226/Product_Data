...
class HfInferStrategy(InferenceStrategy):
    ...
    def generate(self, batch: DataProto, generation_config):
        ...
        all_sequences = []
        all_scores = []  # 用于存储每个 micro-batch 的 scores (logits)
        ...
        for data in micro_batches:
            ...
            if hasattr(res, "sequences"):
                # 如果返回的是 ModelOutput 对象 (因为 return_dict_in_generate=True)
                all_sequences.append(res.sequences)
                if hasattr(res, "scores") and res.scores is not None:
                    # res.scores 是一个 tuple，长度为生成的 token 数，每个元素是 [micro_bs, vocab_size]
                    all_scores.append(res.scores)
            else:
                # 返回的是普通的 torch.Tensor
                all_sequences.append(res)

        # 1. 拼接序列 ID (处理 padding)
        # 使用原来的 pad_sequence 逻辑或直接 cat (HF generate 内部通常已经 pad 过了)
        output_ids = torch.cat(all_sequences, dim=0)
        # 2. 如果没有分数，直接返回 Tensor (保持兼容性)
        if not all_scores:
            return output_ids

        # 3. 拼接 Scores (Logits)
        # all_scores 的结构是: [mb1_scores, mb2_scores, ...]
        # 其中 mb1_scores 是: (step1_logits, step2_logits, ...)
        num_gen_steps = len(all_scores[0])
        concatenated_scores = []
        for i in range(num_gen_steps):
            # 将所有 micro-batch 在第 i 个生成步的 logits 拼在一起
            step_logits = torch.cat([mb[i] for mb in all_scores], dim=0)
            concatenated_scores.append(step_logits)
        # 4. 封装成 ModelOutput 返回，这样 Worker 里的 hasattr(output, "scores") 就能生效
        from transformers.utils import ModelOutput
        return ModelOutput(
            sequences=output_ids,
            scores=tuple(concatenated_scores)
        )
